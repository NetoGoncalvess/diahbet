# Script que APAGA todos os jogos existentes e cadastra os 10 jogos de novo, do zero.
# Evita duplicidade - roda com seguranca mesmo se ja tiver jogos cadastrados antes.
# Rode isso no PowerShell (nao precisa estar na pasta do projeto)

$baseUrl = "https://diahbet.onrender.com/api/jogos"

Write-Host "Apagando todos os jogos existentes..." -ForegroundColor Yellow
try {
    Invoke-RestMethod -Uri $baseUrl -Method Delete | Out-Null
    Write-Host "Jogos apagados com sucesso." -ForegroundColor Green
} catch {
    Write-Host "Erro ao apagar jogos (talvez o servidor ainda nao tenha essa rota atualizada - verifique se fez o deploy mais recente): $_" -ForegroundColor Red
    Write-Host "Abortando para nao cadastrar duplicado." -ForegroundColor Red
    exit
}
...
Write-Host "`nCadastrando jogos..." -ForegroundColor Yellow
foreach ($jogo in $jogos) {
    try {
        $body = $jogo | ConvertTo-Json
        Invoke-RestMethod -Uri $baseUrl -Method Post -Body $body -ContentType "application/json; charset=utf-8" | Out-Null
